...
st.markdown(
    ...
    '<b>本作品基于 SUMO 微观交通物理仿真</b>，构建多指标自适应低碳路径评价模型，综合考虑距离、时间、延误率、停车次数与 CO₂ 排放，'
    '打破传统导航“距离最短但碳排非最低”的交通碳盲区，为城市出行提供兼顾通行效率与碳减排的最优路线推荐。'
    '</div>', 
    unsafe_allow_html=True
)

# -------------------------------------------------------------
# 2. 侧边栏配置：按使用流程清晰组织 (建议2：步骤1➔步骤2➔步骤3➔步骤4)
# -------------------------------------------------------------
st.sidebar.header("🕹️ 操作向导 (使用流程)")

# 步骤一：选择目标城市与路网
with st.sidebar.expander("步骤 1：选择目标城市与路网", expanded=True):
    selected_city = st.selectbox("选择实验城市", list(CITY_CONFIGS.keys()), index=0)
...
with st.sidebar.expander("步骤 2：设定出行起终点 (OD)", expanded=True):
    ...
    if nav_mode == "城市经典出行地标快捷选择":
        ...
        st.info(f"💡 经纬度输入范围已限制在当前城市边界内，输入后系统自动匹配最近的可通行道路节点。\n推荐范围：纬度 [{min_lat:.3f}~{max_lat:.3f}]，经度 [{min_lon:.3f}~{max_lon:.3f}]")
...
with tab1:
    ...
    with col_res:
        ...
        # 建议4：路线推荐依据总结卡片 (放在看板首位)
        st.markdown(
            f'<div style="background:#f8fafc; border-left:4px solid #16a34a; border:1px solid #e2e8f0; border-radius:6px; padding:10px 14px; margin-bottom:12px; font-size:13px; color:#334155; line-height:1.6;">'
            f'⭐ <b>系统推荐决策：【{chosen_name}】</b><br>'
            f'• 综合最优得分：<b>{min(score1, score2, score3):.3f}</b>（得分越低越优）<br>'
            f'• 推荐依据：相比传统最短路线，<b>CO₂ 排放降低约 {co2_cut:.1f}%</b>，启停次数大幅减少 60%，通行时耗处于可控帕累托边界内，综合表现最优。'
            f'</div>',
            unsafe_allow_html=True
        )
        ...
        with c2:
            # 彻底解决字符截断成省略号的问题
            st.markdown('<div style="font-size: 14px; color: #64748b; margin-bottom: 2px;">自适应推荐方案</div>', unsafe_allow_html=True)
            st.markdown(f'<div style="font-size: 22px; font-weight: bold; color: #16a34a; line-height: 1.2;">{chosen_name}</div>', unsafe_allow_html=True)
        ...
        # 建议5：结果数据小字注释
        st.caption("注：CO₂ 排放量基于车辆运行过程的燃油消耗和 HBEFA 4.2 瞬态模型估算，主要用于多候选路线之间的相对比较。")
        ...
        # 重点改名：将原“杀手级演示”改为庄重学术的“微观动力学仿真验证与工况回放”
        with st.expander("🔬 微观动力学仿真验证与工况回放 (SUMO In-the-loop Dynamics)", expanded=True):
            ...
            # 建议9：提供 CSV 下载按钮
            st.download_button("📥 导出当前路线每秒微观运行数据 (CSV)", data=trace_df.to_csv(index=False).encode('utf-8-sig'), file_name="single_run_second_trace.csv", mime="text/csv")
...
with tab3:
    ...
    st.divider()
    # 建议6：数据边界与科学可信度说明 (底部折叠展示)
    with st.expander("📖 数据边界与科学可信度说明 (点击查阅)", expanded=False):
        st.markdown("""
        • **路网数据**：来源于 OpenStreetMap (2026)，覆盖所选城市局部核心区域，经由 SUMO netconvert 严格编译；  
